...
async function main() {
  ...
  console.log("⚠️  NOTE: This demo uses MOCK PREDICTIONS for demonstration.");
  console.log("   In a real run, the LLM would generate predictions from personas.");
  console.log("   This proves the evaluation framework works on all dates.\n");
  ...
  const report = {
    ...
    note: "MOCK DATA - This demo uses simulated random predictions to showcase the evaluation framework. Real runs use LLM-generated persona predictions.",
    summary: {
  ...
  console.log(`📋 DETAILED RESULTS BY DATE\n`);

  successfulRuns.forEach((result) => {
    console.log(`${result.date}:`);
    console.log(`  Top predicted: ${result.predictions[0]?.program} (${result.predictions[0]?.predicted}%)`);
    if (result.evaluation.perProgramDelta.length > 0) {
      const topActual = result.evaluation.perProgramDelta.sort((a, b) => b.actual - a.actual)[0];
      console.log(`  Actual #1:     ${topActual.programName} (${topActual.actual}%)`);
    }
    console.log(`  Metrics:       MAE=${result.mae}% | ρ=${result.spearmanRho} | Top-1=${result.top1Hit ? "✓" : "✗"}`);
    console.log();
  });

  console.log(`\n📄 Full results saved to: data/tv-audience/results/${filename}\n`);

  console.log(`================== TO RUN WITH REAL LLM PREDICTIONS ==================\n`);
  console.log(`Option 1: Via API (requires running server)`);
  console.log(`   npm run dev`);
  console.log(`   curl -X POST http://localhost:3000/api/lab/batch-eval\n`);
  console.log(`Option 2: Via TypeScript runner`);
